...
function generateToken(prefix = "") {
  ...
  const base = btoa(String.fromCharCode(...bytes)).replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
  return `${prefix}${base}`;
}
...
var adminAuth = createMiddleware(async (c, next) => {
  ...
  if (new Date(String(session.expires_at)).getTime() <= Date.now()) {
    await c.env.DB.prepare("DELETE FROM admin_sessions WHERE id = ?").bind(String(session.id)).run();
...
var resolveScheduleParts = /* @__PURE__ */ __name((value) => parseScheduleTime(value) ?? DEFAULT_SCHEDULE_PARTS, "resolveScheduleParts");
function computeBeijingScheduleTime(now, scheduleTime) {
  ...
  const utcMs = Date.UTC(year, month - 1, day, hour, minute, 0) - BEIJING_OFFSET_MS;
...
function computeNextBeijingRun(now, scheduleTime) {
  ...
  const { year, month, day } = getBeijingDateParts(now);
  const { hour, minute } = resolveScheduleParts(scheduleTime);
  const utcMs = Date.UTC(year, month - 1, day + 1, hour, minute, 0) - BEIJING_OFFSET_MS;
  return new Date(utcMs);
}
...
async function readSetting(db, key) {
  const setting = await db.prepare("SELECT value FROM settings WHERE key = ?").bind(key).first();
...
async function bumpCacheVersion(db, scope) {
  ...
  ).bind(key, String(DEFAULT_CACHE_VERSION + 1), nowIso()).run();
}
...
async function getCheckinScheduleTime(db) {
  return getCachedSetting(
    ...
    async () => {
      ...
      return timeRaw && timeRaw.length > 0 ? timeRaw : DEFAULT_CHECKIN_SCHEDULE_TIME;
    },
    (next) => {
...
auth.post("/login", async (c) => {
  ...
  const expiresAt = addHours(/* @__PURE__ */ new Date(), ttlHours).toISOString();
  await c.env.DB.prepare(
    "INSERT INTO admin_sessions (id, token_hash, expires_at, created_at) VALUES (?, ?, ?, ?)"
  ).bind(crypto.randomUUID(), tokenHash, expiresAt, nowIso()).run();
  return c.json({
    token: rawToken,
...
auth.post("/logout", adminAuth, async (c) => {
  ...
  if (sessionId) {
    await c.env.DB.prepare("DELETE FROM admin_sessions WHERE id = ?").bind(String(sessionId)).run();
  }
...
async function deleteCallTokensByChannelId(db, channelId) {
  await db.prepare("DELETE FROM channel_call_tokens WHERE channel_id = ?").bind(channelId).run();
}
...
async function updateCallTokenModels(db, tokenId, models2, updatedAt) {
  ...
  ).bind(modelsJson, updatedAt, tokenId).run();
}
...
function normalizeModelsInput(input) {
  ...
  if (typeof input === "object") {
    ...
    if (Array.isArray(raw2.data)) {
      return raw2.data.map((item) => toModelId(item)).filter((item) => item.length > 0);
    }
  }
  return [];
}
...
function modelsToJson(models2) {
  const normalized = models2.map((model) => String(model).trim()).filter((model) => model.length > 0);
  return JSON.stringify(normalized.map((id) => ({ id })));
}
__name(modelsToJson, "modelsToJson");
function extractModelIds(channel) {
  const raw2 = safeJsonParse(
    channel.models_json,
    null
  );
  const models2 = Array.isArray(raw2) ? raw2 : Array.isArray(raw2?.data) ? raw2.data : [];
  return models2.map((model) => toModelId(model)).filter((model) => model.length > 0);
}
...
async function listChannels(db, options = {}) {
  ...
  const limitSql = options.limit !== void 0 && options.offset !== void 0 ? "LIMIT ? OFFSET ?" : "";
  const limitBindings = options.limit !== void 0 && options.offset !== void 0 ? [options.limit, options.offset] : [];
  const statement = db.prepare(
...
async function countChannelsByType(db, filters) {
  ...
  for (const entry of counts.results ?? []) {
    result[String(entry.type)] = Number(
      entry.count ?? 0
    );
  }
  return result;
...
async function listActiveChannels(db) {
  const rows = await db.prepare("SELECT * FROM channels WHERE status = ?").bind("active").all();
  return rows.results ?? [];
}
...
async function channelExists(db, id) {
  const row = await db.prepare("SELECT id FROM channels WHERE id = ?").bind(id).first();
...
async function insertChannel(db, input) {
  await db.prepare(
    ...
    typeof input.checkin_enabled === "boolean" ? input.checkin_enabled ? 1 : 0 : input.checkin_enabled ?? 0,
...
async function listCoolingDownChannelsForModel(db, channelIds, model, cooldownSeconds, minErrorCount = 1) {
  ...
  ).bind(model, ...channelIds, cutoff).all();
...
async function recordChannelModelError(db, channelId, model, errorCode, nowSeconds = Math.floor(Date.now() / 1e3)) {
  if (!model) {
    return;
  }
  ...
  ).bind(channelId, model, nowSeconds, errorCode, timestamp, timestamp).run();
}
__name(recordChannelModelError, "recordChannelModelError");
async function upsertChannelModelCapabilities(db, channelId, models2, nowSeconds = Math.floor(Date.now() / 1e3)) {
  if (models2.length === 0) {
    return;
  }
  const timestamp = nowIso();
  ...
  const statements = models2.map(
    (model) => stmt.bind(channelId, model, nowSeconds, timestamp, timestamp)
  );
...
async function testChannelTokens(baseUrl, tokens2, fetcher = fetchChannelModels) {
  if (tokens2.length === 0) {
    return {
      ok: false,
      total: 0,
...
async function updateChannelTestResult(db, id, result) {
  const now = Math.floor(Date.now() / 1e3);
  const current = await db.prepare("SELECT status FROM channels WHERE id = ?").bind(id).first();
  const currentStatus = current?.status ?? "active";
  const status = currentStatus === "disabled" ? "disabled" : result.ok ? "active" : "error";
  const modelsJson = result.modelsJson ?? (result.models ? modelsToJson(result.models) : void 0);
  const sql = modelsJson ? "UPDATE channels SET status = ?, models_json = ?, test_time = ?, response_time_ms = ?, updated_at = ? WHERE id = ?" : "UPDATE channels SET status = ?, test_time = ?, response_time_ms = ?, updated_at = ? WHERE id = ?";
...
channels.post("/:id/test", async (c) => {
  ...
  const tokens2 = callTokenRows.length > 0 ? callTokenRows.map((row) => ({
    id: row.id,
    name: row.name,
    api_key: row.api_key
  })) : [
...
function buildDateFilters(query) {
  let sql = " WHERE 1=1";
  const params = [];
  const channelIds = (query.channel_ids ?? "").split(",").map((value) => value.trim()).filter(Boolean);
  const tokenIds = (query.token_ids ?? "").split(",").map((value) => value.trim()).filter(Boolean);
  if (query.from) {
...
dashboard.get("/", async (c) => {
  ...
  return withApiCache(
    ...
    async () => {
      ...
      const interval = query.interval === "week" || query.interval === "month" ? query.interval : "day";
      ...
      const bucketExpression = interval === "week" ? "strftime('%Y-W%W', created_at)" : interval === "month" ? "substr(created_at, 1, 7)" : "substr(created_at, 1, 10)";
...
function toInternalStatus(status) {
  ...
  if (typeof status === "string") {
    ...
    if (["0", "2", "false", "disabled", "disable", "inactive"].includes(
      normalized
    )) {
      return "disabled";
    }
...
function normalizeChannelInput(body) {
  const hasModels = body && (Object.hasOwn(body, "models") || Object.hasOwn(body, "model") || Object.hasOwn(body, "model_list") || Object.hasOwn(body, "models_list"));
  const models2 = hasModels ? normalizeModelsInput(
    body?.models ?? body?.model ?? body?.model_list ?? body?.models_list
  ) : [];
  const hasGroup = body && (Object.hasOwn(body, "group") || Object.hasOwn(body, "group_name") || Object.hasOwn(body, "groups"));
  const groupInput = body?.group ?? body?.group_name ?? null;
  const groupsInput = Array.isArray(body?.groups) ? body.groups.map((item) => String(item)) : null;
  const groupName = !hasGroup ? void 0 : groupInput ? String(groupInput) : groupsInput && groupsInput.length > 0 ? groupsInput.join(",") : null;
  const metadata = {};
  ...
  const hasRateLimit = body && (Object.hasOwn(body, "rate_limit") || Object.hasOwn(body, "rateLimit"));
  ...
  return {
    ...
    base_url: body?.base_url !== void 0 && body?.base_url !== null ? String(body.base_url) : body?.baseUrl !== void 0 && body?.baseUrl !== null ? String(body.baseUrl) : void 0,
    api_key: body?.key !== void 0 && body?.key !== null ? String(body.key) : body?.api_key !== void 0 && body?.api_key !== null ? String(body.api_key) : void 0,
    weight: hasWeight ? toNumber(body?.weight, 1) ?? 1 : void 0,
    status: hasStatus ? toInternalStatus(body?.status) : void 0,
    rate_limit: hasRateLimit ? toNumber(body?.rate_limit ?? body?.rateLimit, 0) ?? 0 : void 0,
    models: models2,
...
function toNewApiChannel(channel) {
  ...
  return {
    ...
    key: channel.api_key,
    created_time: Number.isNaN(createdTime) ? void 0 : Math.floor(createdTime / 1e3),
    updated_time: Number.isNaN(updatedTime) ? void 0 : Math.floor(updatedTime / 1e3),
    ...metadata
  };
}
...
newapi.get("/search", async (c) => {
  ...
  const filtered = rows.filter((row) => {
    const channel = row;
    const models2 = extractModelIds(channel);
    if (keyword && !String(channel.name).includes(keyword) && !String(channel.id).includes(keyword)) {
      return false;
    }
...
newapi.put("/tag", async (c) => {
  ...
  const nextTag = body.new_tag !== void 0 && body.new_tag !== null ? String(body.new_tag).trim() : null;
  const nextWeight = body.weight !== void 0 && body.weight !== null ? Number(body.weight) : null;
  const nextPriority = body.priority !== void 0 && body.priority !== null ? Number(body.priority) : null;
  const rows = await listChannels(c.env.DB);
  ...
  for (const row of targets) {
    ...
    const mergedMetadata = Object.keys(metadata).length > 0 ? JSON.stringify(metadata) : null;
    const weight = nextWeight !== null && !Number.isNaN(nextWeight) ? nextWeight : row.weight ?? 1;
    const priority = nextPriority !== null && !Number.isNaN(nextPriority) ? nextPriority : row.priority ?? 0;
    await c.env.DB.prepare(
      "UPDATE channels SET weight = ?, priority = ?, metadata_json = ?, updated_at = ? WHERE id = ?"
    ).bind(weight, priority, mergedMetadata, nowIso(), row.id).run();
  }
  await bumpCacheVersions(c.env.DB, ["channels", "models"]);
  return newApiSuccess(c);
});
...
newapi.post("/tag/disabled", async (c) => {
  ...
  for (const row of targets) {
    await c.env.DB.prepare(
      "UPDATE channels SET status = ?, updated_at = ? WHERE id = ?"
    ).bind("disabled", nowIso(), row.id).run();
  }
  await bumpCacheVersions(c.env.DB, ["channels", "models"]);
  return newApiSuccess(c);
});
...
newapi.put("/", async (c) => {
  ...
  const parsed = normalizeChannelInput(payload);
  const models2 = parsed.models.length > 0 ? parsed.models : extractModelIds(current);
  const mergedMetadata = mergeMetadata(
    current.metadata_json,
    parsed.metadata_json
  );
  const nextBaseUrl = normalizeBaseUrlInput(parsed.base_url ?? current.base_url) ?? String(current.base_url);
...
newapi.get("/:id", async (c) => {
  ...
  const modelMapping = metadata.model_mapping === void 0 || metadata.model_mapping === null ? "{}" : String(metadata.model_mapping);
  const channelInfo = metadata.channel_info && typeof metadata.channel_info === "object" && !Array.isArray(metadata.channel_info) ? metadata.channel_info : {
    is_multi_key: false,
...
function collectGroups(rows) {
  ...
  for (const row of rows) {
    ...
    raw2.split(",").map((item) => item.trim()).filter((item) => item.length > 0).forEach((item) => {
      names.add(item);
    });
  }
  if (names.size === 0) {
...
var tokenAuth = createMiddleware(async (c, next) => {
  ...
  const quotaTotal = record.quota_total === null || record.quota_total === void 0 ? null : Number(record.quota_total);
...
function parseSiteMetadata(raw2) {
  ...
  const site_type = rawType === "done-hub" || rawType === "new-api" || rawType === "subapi" || rawType === "openai" || rawType === "anthropic" || rawType === "gemini" ? rawType : rawType === "custom" ? "subapi" : DEFAULT_SITE_TYPE;
  const overrides = parsed.endpoint_overrides && typeof parsed.endpoint_overrides === "object" ? parsed.endpoint_overrides : {};
  return {
    site_type,
    endpoint_overrides: {
...
function build_upstream_chat_request(payload_json, provider, model, endpoint, is_stream, endpoint_overrides_json) {
  let deferred6_0;
  let deferred6_1;
  try {
    const ptr0 = passStringToWasm0(payload_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
...
function normalize_chat_request(payload_json, provider, endpoint, model, is_stream) {
  let deferred5_0;
  let deferred5_1;
  try {
    const ptr0 = passStringToWasm0(payload_json, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
...
var normalizeUsageViaWasm = /* @__PURE__ */ __name((raw2) => safeJsonParse(normalize_usage(toJson(raw2)), null), "normalizeUsageViaWasm");
var parseUsageFromJsonViaWasm = /* @__PURE__ */ __name((payload) => safeJsonParse(
  parse_usage_from_json(toJson(payload)),
  null
), "parseUsageFromJsonViaWasm");
var parseUsageFromSseLineViaWasm = /* @__PURE__ */ __name((line) => safeJsonParse(
  parse_usage_from_sse_line(line),
  null
), "parseUsageFromSseLineViaWasm");
var createWeightedOrderIndicesViaWasm = /* @__PURE__ */ __name((weights) => safeJsonParse(
  create_weighted_order(toJson(weights), randomSeed()),
  null
), "createWeightedOrderIndicesViaWasm");
var detectDownstreamProviderViaWasm = /* @__PURE__ */ __name((path) => detect_downstream_provider(path), "detectDownstreamProviderViaWasm");
var detectEndpointTypeViaWasm = /* @__PURE__ */ __name((provider, path) => detect_endpoint_type(provider, path), "detectEndpointTypeViaWasm");
var parseDownstreamModelViaWasm = /* @__PURE__ */ __name((provider, path, body) => {
  const output = parse_downstream_model(provider, path, toJson(body ?? {}));
  return output.length > 0 ? output : null;
}, "parseDownstreamModelViaWasm");
var parseDownstreamStreamViaWasm = /* @__PURE__ */ __name((provider, path, body) => parse_downstream_stream(provider, path, toJson(body ?? {})), "parseDownstreamStreamViaWasm");
var applyGeminiModelToPathViaWasm = /* @__PURE__ */ __name((path, model) => apply_gemini_model_to_path(path, model ?? ""), "applyGeminiModelToPathViaWasm");
var normalizeChatRequestViaWasm = /* @__PURE__ */ __name((payload, provider, endpoint, model, isStream) => safeJsonParse(
...
var buildUpstreamChatRequestViaWasm = /* @__PURE__ */ __name((payload, provider, model, endpoint, isStream, endpointOverrides) => safeJsonParse(
...
var adaptChatJsonViaWasm = /* @__PURE__ */ __name((direction, payload, model) => safeJsonParse(
  adapt_chat_json(
    direction,
    toJson(payload),
    model ?? "",
...
var adaptSseLineViaWasm = /* @__PURE__ */ __name((payload, upstream, downstream, model) => safeJsonParse(
...
function createWeightedOrder(channels2) {
  ...
  const mapped = wasmOrder.map((index) => pool[index]).filter((item) => Boolean(item));
...
function writeGeminiChunk(controller, encoder2, data) {
  controller.enqueue(encoder2.encode(`${JSON.stringify(data)}
`));
}
__name(writeGeminiChunk, "writeGeminiChunk");
async function adaptOpenAiJsonToAnthropic(options) {
  const payload = await options.response.clone().json().catch(() => null);
  if (!payload) {
      ...
      try {
        while (true) {
          ...
          while (newlineIndex !== -1) {
            ...
            const stopReason = typeof wasmLine.stopReason === "string" ? wasmLine.stopReason : null;
      ...
      try {
        while (true) {
          ...
          while (newlineIndex !== -1) {
            ...
            const eventType = typeof wasmLine.eventType === "string" ? wasmLine.eventType : "";
            ...
            if (eventType === "message_delta" && !stopSent) {
              const finishReason = typeof wasmLine.finishReason === "string" ? wasmLine.finishReason : null;
              writeOpenAiSseChunk(controller, encoder2, {
                id: completionId,
                object: "chat.completion.chunk",
                created,
          ...
          while (newlineIndex !== -1) {
            ...
            if (stopReason && !stopped) {
              ...
              writeSseEvent(controller, encoder2, "message_delta", {
                ...
                delta: { stop_reason: stopReason, stop_sequence: null },
                usage: {
                  output_tokens: typeof wasmLine.outputTokens === "number" ? wasmLine.outputTokens : 0
                }
              });
              writeSseEvent(controller, encoder2, "message_stop", {
                type: "message_stop"
        ...
        while (true) {
          ...
          while (newlineIndex !== -1) {
            ...
            if (eventType === "message_delta") {
              lastFinishReason = (typeof wasmLine.finishReason === "string" ? wasmLine.finishReason : null) ?? lastFinishReason;
            }
            newlineIndex = buffer.indexOf("\n");
          }
        }
...
function detectEndpointType(provider, path) {
  ...
  if (endpoint === "chat" || endpoint === "responses" || endpoint === "embeddings" || endpoint === "images" || endpoint === "passthrough") {
    return endpoint;
  }
...
function normalizeChatRequest(provider, endpoint, body, model, isStream) {
  return normalizeChatRequestViaWasm(
    body,
    provider,
    endpoint,
    model,
...
function normalizeEmbeddingRequest(provider, body, model) {
  ...
  if (provider === "gemini") {
    if (Array.isArray(body.requests)) {
      const inputs = body.requests.map((req) => {
...
function normalizeImageRequest(provider, body, model) {
  ...
  if (provider === "openai") {
    return {
      ...
      responseFormat: body.response_format ? String(body.response_format) : null
    };
  }
  return {
    model,
...
function buildUpstreamChatRequest(provider, normalized, model, endpoint, isStream, endpointOverrides) {
...
function buildUpstreamEmbeddingRequest(provider, normalized, model, endpointOverrides) {
  if (provider === "openai") {
    ...
    return {
      ...
      body: {
        model,
        input: normalized.inputs.length === 1 ? normalized.inputs[0] : normalized.inputs
      }
    };
  }
  ...
  const defaultPath = isBatch ? `/v1beta/models/${model}:batchEmbedContents` : `/v1beta/models/${model}:embedContent`;
  const body = isBatch ? {
...
function buildUpstreamImageRequest(provider, normalized, model, endpointOverrides) {
  if (provider === "openai") {
...
var getUsageLimiterStub = /* @__PURE__ */ __name((namespace) => namespace.get(namespace.idFromName(LIMITER_NAME)), "getUsageLimiterStub");
...
async function recordUsage(db, input) {
  ...
  const streamValue = input.stream === null || input.stream === void 0 ? null : typeof input.stream === "number" ? input.stream : input.stream ? 1 : 0;
  const reasoningValue = input.reasoningEffort === null || input.reasoningEffort === void 0 ? null : String(input.reasoningEffort);
  await db.prepare(
  ...
  if (input.tokenId && input.totalTokens) {
    ...
    ).bind(input.totalTokens, createdAt, input.tokenId).run();
  }
}
__name(recordUsage, "recordUsage");
async function pruneUsageLogs(db, retentionDays) {
  const now = Date.now();
  if (lastPruneRetention === retentionDays && now - lastPruneAt < PRUNE_INTERVAL_MS) {
    return;
  }
  ...
  await db.prepare("DELETE FROM usage_logs WHERE created_at < ?").bind(cutoff.toISOString()).run();
}
...
async function parseUsageFromSse(response, options = {}) {
  ...
  const maxBytes = typeof options.maxBytes === "number" && options.maxBytes > 0 ? options.maxBytes : Number.POSITIVE_INFINITY;
...
function getStreamUsageMaxParsers(settings2) {
  const maxParsers = Math.max(0, Math.floor(settings2.stream_usage_max_parsers));
  if (maxParsers === 0) {
...
function createUsageEventScheduler(c, settings2) {
  ...
  const limiter = c.env.USAGE_LIMITER ? getUsageLimiterStub(c.env.USAGE_LIMITER) : null;
...
async function extractErrorDetails(response) {
  ...
  if (contentType.includes("application/json")) {
    ...
    if (payload && typeof payload === "object") {
      ...
      const errorCode = typeof error.code === "string" ? error.code : typeof error.type === "string" ? error.type : null;
      const errorMessage = typeof error.message === "string" ? error.message : typeof raw2.message === "string" ? raw2.message : null;
      return {
        errorCode,
...
function channelSupportsModel(channel, model, verifiedModelsByChannel) {
  ...
  const declaredAllows = declaredModels.length > 0 ? (mapped ? declaredModels.includes(mapped) : false) || declaredModels.includes(model) : null;
  const verifiedAllows = verified && verified.size > 0 ? (mapped ? verified.has(mapped) : false) || verified.has(model) : null;
...
function selectCandidateChannels(allowedChannels, downstreamModel, verifiedModelsByChannel = /* @__PURE__ */ new Map()) {
  const modelChannels = allowedChannels.filter(
    (channel) => channelSupportsModel(channel, downstreamModel, verifiedModelsByChannel)
  );
...
function hasExplicitModelMapping(metadata, downstreamModel) {
  if (downstreamModel) {
    return metadata.model_mapping[downstreamModel] !== void 0 || metadata.model_mapping["*"] !== void 0;
  }
  return metadata.model_mapping["*"] !== void 0;
}
__name(hasExplicitModelMapping, "hasExplicitModelMapping");
function resolveUpstreamModelForChannel(channel, metadata, downstreamModel, verifiedModelsByChannel = /* @__PURE__ */ new Map()) {
  ...
  const verified = verifiedModelsByChannel.get(channel.id);
  const declaredModels = verified ? Array.from(verified) : extractModels(channel).map((entry) => entry.id);
  if (declaredModels.length === 0) {
    return { model: mapped, autoMapped: false };
  }
...
var selectTokenForModel = /* @__PURE__ */ __name((tokens2, model) => {
  ...
  const match2 = tokensWithModels.find((entry) => entry.models?.includes(model));
...
function mergeQuery(base, querySuffix, overrides) {
  ...
  if (querySuffix) {
    const suffix = querySuffix.startsWith("?") ? querySuffix.slice(1) : querySuffix;
...
proxy.all("/*", tokenAuth, async (c) => {
  ...
  if (downstreamProvider === "openai" && isStream && parsedBody && typeof parsedBody === "object") {
  ...
  const querySuffix = c.req.url.includes("?") ? `?${c.req.url.split("?")[1]}` : "";
  ...
  for (const channel of ordered) {
    ...
    if (upstreamProvider === "gemini" && !upstreamModel && endpointType !== "passthrough") {
      continue;
    }
    ...
    if (endpointType === "passthrough") {
      ...
      if (endpointType === "chat" || endpointType === "responses") {
        if (metadata.endpoint_overrides.chat_url && normalizedChat) {
          ...
          if (request) {
            ...
            upstreamFallbackPath = request.absoluteUrl ? void 0 : request.fallbackPath;
          }
        } else if (endpointType === "responses" && upstreamProvider === "openai") {
          upstreamFallbackPath = "/responses";
        }
      }
      if (endpointType === "embeddings" && metadata.endpoint_overrides.embedding_url) {
      ...
      upstreamFallbackPath = built.request.absoluteUrl ? void 0 : built.request.fallbackPath;
    ...
    try {
      ...
      if ((response.status === 400 || response.status === 404) && upstreamFallbackPath) {
        const fallbackTargetBase = absoluteUrl ? absoluteUrl : `${baseUrl}${upstreamFallbackPath}`;
      ...
      if (downstreamModel && downstreamModel !== recordModel && cooldownSeconds > 0 && cooldownEligible) {
        scheduleUsageEvent({
          type: "model_error",
          payload: {
            channelId: channel.id,
      ...
      const isTimeout = error instanceof Error && (error.name === "AbortError" || error.message.includes("upstream_timeout"));
  ...
  if (selectedChannel && isStream) {
    ...
    const canParseStream = streamUsageOptions.mode !== "off" && activeStreamUsageParsers < streamUsageMaxParsers;
    if (!canParseStream) {
      recordAttemptUsage({
...
var parseSigned = /* @__PURE__ */ __name((payload) => {
  ...
  return Boolean(
    record.signed ?? record.is_signed ?? record.checked ?? record.checkin ?? record.already_signed ?? record.checked_in
  );
}, "parseSigned");
...
async function runCheckin(site) {
  ...
  try {
    ...
    const numericCode = typeof codeValue === "number" ? codeValue : typeof codeValue === "string" ? Number(codeValue) : null;
    const explicitFailure = Boolean(
      record.success === false || record.status === "error" || record.error && String(record.error).length > 0 || numericCode !== null && !Number.isNaN(numericCode) && numericCode !== 0
    );
    if (explicitFailure) {
    ...
    return {
      ...
      checkin_date: extractCheckinDate(checkinPayload) ?? extractCheckinDate(verifyPayload)
    };
  } catch (error) {
    return {
      id: site.id,
...
async function runCheckinAll(db, now = /* @__PURE__ */ new Date()) {
  ...
  for (const channel of channelRows) {
    ...
    const checkinEnabled = typeof rawEnabled === "boolean" ? rawEnabled : Number(rawEnabled) === 1;
    if (!checkinEnabled) {
      continue;
    }
    const alreadyChecked = channel.last_checkin_date === today && (channel.last_checkin_status === "success" || channel.last_checkin_status === "skipped");
    if (alreadyChecked) {
      results.push({
        id: channel.id,
        name: channel.name,
...
async function runCheckinSingle(db, channelId, now = /* @__PURE__ */ new Date()) {
...
var getCheckinSchedulerStub = /* @__PURE__ */ __name((namespace) => namespace.get(namespace.idFromName(SCHEDULER_NAME)), "getCheckinSchedulerStub");
var shouldRunCheckin = /* @__PURE__ */ __name((now, scheduleTime, lastRunDate) => {
  const today = beijingDateString(now);
...
var shouldResetLastRun = /* @__PURE__ */ __name((currentTime, nextTime) => currentTime !== nextTime, "shouldResetLastRun");
...
var CheckinScheduler = class {
  ...
  async fetch(request) {
    ...
    if (request.method === "GET" && url.pathname === "/status") {
      const lastRunDate = await this.state.storage.get(LAST_RUN_DATE_KEY) ?? null;
      return new Response(
        JSON.stringify({
          ok: true,
...
settings.put("/", async (c) => {
  ...
  if (body.proxy_retry_max_retries !== void 0) {
    ...
    if (Number.isNaN(retryMaxRetries) || retryMaxRetries < 0 || !Number.isInteger(retryMaxRetries)) {
      return jsonError(
        c,
        400,
...
var parseSiteType = /* @__PURE__ */ __name((value) => {
  if (value === "done-hub" || value === "new-api" || value === "subapi" || value === "openai" || value === "anthropic" || value === "gemini") {
    return value;
  }
  if (value === "custom") {
    return "subapi";
...
var normalizeCallTokens = /* @__PURE__ */ __name((rawTokens, fallbackApiKey) => {
  const tokens2 = rawTokens?.map((token, index) => ({
...
var toCallTokenRows = /* @__PURE__ */ __name((channelId, tokens2, now) => tokens2.map((token) => ({
...
sites.get("/", async (c) => {
  ...
  const sitesList = channels2.map((channel) => {
    const tokens2 = callTokenMap.get(channel.id) ?? [];
    const callTokens = tokens2.length > 0 ? tokens2 : channel.api_key ? [
      {
        id: "",
        name: "\u4E3B\u8C03\u7528\u4EE4\u724C",
        api_key: channel.api_key
...
sites.post("/", async (c) => {
  ...
  const checkinUrl = body.checkin_url !== void 0 && body.checkin_url !== null ? trimValue(body.checkin_url) : "";
  const checkinEnabled = siteType === "new-api" ? parseBoolean(body.checkin_enabled, body.checkin_status === "active") : false;
...
sites.patch("/:id", async (c) => {
  ...
  const nextSiteType = body.site_type ? parseSiteType(body.site_type) : currentMetadata.site_type;
  const baseUrl = body.base_url !== void 0 ? resolveBaseUrl(nextSiteType, body.base_url) : normalizeBaseUrl(String(current.base_url));
  if (!baseUrl) {
    return jsonError(c, 400, "missing_base_url", "missing_base_url");
  }
  const shouldUpdateTokens = body.call_tokens !== void 0 || body.api_key !== void 0;
  const callTokens = shouldUpdateTokens ? normalizeCallTokens(body.call_tokens, body.api_key ?? current.api_key) : [];
  if (shouldUpdateTokens && callTokens.length === 0) {
    return jsonError(c, 400, "missing_call_tokens", "missing_call_tokens");
  }
  const metadataJson = body.site_type !== void 0 ? buildSiteMetadata(current.metadata_json, {
    site_type: nextSiteType
  }) : current.metadata_json ?? null;
  const nextSystemToken = body.system_token !== void 0 || body.checkin_token !== void 0 ? trimValue(body.system_token ?? body.checkin_token) : trimValue(current.system_token ?? "");
  const nextSystemUser = body.system_userid !== void 0 || body.checkin_userid !== void 0 ? trimValue(body.system_userid ?? body.checkin_userid) : trimValue(current.system_userid ?? "");
  const nextCheckinUrl = body.checkin_url !== void 0 ? body.checkin_url !== null ? trimValue(body.checkin_url) : "" : trimValue(current.checkin_url ?? "");
  const currentCheckinEnabled = typeof current.checkin_enabled === "boolean" ? current.checkin_enabled : Number(current.checkin_enabled ?? 0) === 1;
  const nextCheckinEnabled = nextSiteType === "new-api" ? body.checkin_enabled !== void 0 || body.checkin_status !== void 0 ? parseBoolean(body.checkin_enabled, body.checkin_status === "active") : currentCheckinEnabled : false;
...
sites.post("/:id/checkin", async (c) => {
  const id = c.req.param("id");
  const result = await runCheckinSingle(c.env.DB, id, /* @__PURE__ */ new Date());
  if (!result) {
    return jsonError(c, 404, "site_not_found", "site_not_found");
  }
  return c.json({
...
tokens.post("/", async (c) => {
  ...
  const quotaTotal = body.quota_total === null || body.quota_total === void 0 ? null : Number(body.quota_total);
...
tokens.patch("/:id", async (c) => {
  ...
  const existing = await c.env.DB.prepare("SELECT * FROM tokens WHERE id = ?").bind(id).first();
  if (!existing) {
    return jsonError(c, 404, "token_not_found", "token_not_found");
  }
  const existingAllowed = safeJsonParse(existing.allowed_channels, null);
  const quotaTotalUpdate = body.quota_total === null || body.quota_total === void 0 ? existing.quota_total : Number(body.quota_total);
  const quotaUsedUpdate = body.quota_used === null || body.quota_used === void 0 ? existing.quota_used : Number(body.quota_used);
  ...
  return withApiCache(
    ...
    async () => {
      ...
      if (query.channel_ids) {
        const channelIds = String(query.channel_ids).split(",").map((item) => item.trim()).filter(Boolean);
        if (channelIds.length > 0) {
          const placeholders = channelIds.map(() => "?").join(", ");
      ...
      if (query.token_ids) {
        const tokenIds = String(query.token_ids).split(",").map((item) => item.trim()).filter(Boolean);
        if (tokenIds.length > 0) {
      ...
      if (query.models) {
        const models2 = String(query.models).split(",").map((item) => item.trim()).filter(Boolean);
        if (models2.length > 0) {
      ...
      if (query.statuses) {
        const statuses = String(query.statuses).split(",").map((item) => item.trim()).filter(Boolean);
        if (statuses.length > 0) {
          const numericStatuses = statuses.map((item) => Number(item)).filter((value) => !Number.isNaN(value));
          const textStatuses = statuses.filter(
      ...
      const normalizedOffset = Number.isNaN(rawOffset) ? 0 : Math.max(0, Math.floor(rawOffset));
      const whereSql = filters.length > 0 ? `WHERE ${filters.join(" AND ")}` : "";
      const baseSql = "FROM usage_logs LEFT JOIN channels ON channels.id = usage_logs.channel_id LEFT JOIN tokens ON tokens.id = usage_logs.token_id";
      const retention = await getRetentionDays(c.env.DB);
      ...
      const result = await c.env.DB.prepare(listSql).bind(...params, limit, normalizedOffset).all();
      return c.json({
        logs: result.results ?? [],
        total,
        limit,
...
app.use("/api/*", async (c, next) => {
  if (c.req.path === "/api/auth/login" || c.req.path.startsWith("/api/channel") || c.req.path.startsWith("/api/user") || c.req.path.startsWith("/api/group")) {
    return next();
  }
...
app.onError((err, c) => {
  ...
  if (c.req.path === "/api" || c.req.path.startsWith("/api/") || c.req.path === "/v1" || c.req.path.startsWith("/v1/") || c.req.path === "/v1beta" || c.req.path.startsWith("/v1beta/")) {
